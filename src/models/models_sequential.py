...
def save_tensor_to_npz(tensor, filename):
    """
    Save a PyTorch tensor to a .npz file.
    ...
    np.savez(filename, tensor=np_array)
...
###our 3D pos embed    
def get_3d_sincos_pos_embed(embed_dim, grid_size, depth, cls_token=False, extra_tokens=0):
    """
    grid_size: int of the grid height and width
    return:
    pos_embed: [grid_size*grid_size, embed_dim] or [1+grid_size*grid_size, embed_dim] (w/ or w/o cls_token)
    ...
    grid_h = np.arange(grid_size, dtype=np.float32)
    grid_w = np.arange(grid_size, dtype=np.float32)
    grid_d = np.arange(depth, dtype=np.float32)
    grid = np.meshgrid(grid_d, grid_w, grid_h, indexing='ij')  # here w goes first
    grid = np.stack(grid, axis=0)

    grid = grid.reshape([3, 1, depth, grid_size, grid_size])
